function p = fillCircle( c, xc, yc, r, varargin )
...
%
% Usage:
%   fillCircle( c,xc, yc, r, sang, theta )
%   fillCircle( c,xc, yc, r, sang, theta, '-pie'|'-seg' )
%   fillCircle( c,xc, yc, r, sang, theta, LineSpec)
%   fillCircle( c,xc, yc, r, sang, theta, '-np',40)
%   p = fillCircle(__)
%
% Input:
%   c        -- color
%   xc, yc   -r- circle center
%   r        -r- circle radius (> 0)
%
% Optional input:
%   sang     -r- start angle (deg)
%   theta    -r- central angle (deg), > 0 CCW, < 0 CW
%   '-pie'   --- draw pie
%   '-seg'   --- draw segment
%
...
%
%Optional output
%   p    -- structure containing some output data
%
    narginchk(4,inf)
    nargoutchk(0,1)
